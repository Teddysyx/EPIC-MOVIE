...
function playChannel(streamUrl, channelName) {
    ...
    // Zastavit aktuální video
    video.pause();
    video.src = '';
    
    // Vytvořit nový source element s ?download parametrem
    const source = document.createElement('source');
    source.src = streamUrl + '?download';
    source.type = 'video/mp4';
    
    // Vyčistit a přidat nový zdroj
    video.innerHTML = '';
    ...
    // Pokusit se přehrát - pokud selže, nechat uživateli možnost kliknout na play
    const playPromise = video.play();
    
    if (playPromise !== undefined) {
        playPromise.catch(error => {
            console.log('Automatické přehrávání selhalo:', error);
            // Uživatel bude muset kliknout na play tlačítko ve videu
        });
    }
}
