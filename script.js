...
// Funkce pro výběr náhodného filmu
...
function setRandomHeroMovie() {
    ...
    // Najdeme logo a popis a nastavíme je
...
// Funkce pro přiřazení stream URL k filmům
function assignStreamUrls() {
    featuredMovies.forEach(movie => {
        // Najdeme odpovídající film v M3U playlistu podle názvu
        const matchingChannel = allChannels.find(channel => 
            channel.name.toLowerCase().includes(movie.title.toLowerCase()) ||
            movie.title.toLowerCase().includes(channel.name.toLowerCase())
        );
        
        if (matchingChannel) {
            movie.streamUrl = matchingChannel.url;
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Nastavíme náhodný film při načtení stránky
    setRandomHeroMovie();
    // Načteme playlist
    loadPlaylist();
});
...
// Upravený posluchač pro tlačítko "Přehrát" - přehraje aktuální hero film
