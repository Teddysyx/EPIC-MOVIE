...
async function loadPlaylist() {
    try {
        ...
        movieContainer.innerHTML = '<div class="loading">Načítám seznam filmů...</div>';
        ...
        const movieContainer = document.getElementById('movie-container');
        movieContainer.innerHTML = '<div class="error">Nepodařilo se načíst seznam filmů</div>';
    }
}
...
function parseM3U(m3uText) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('#EXTINF:')) {
            ...
            const logo = logoMatch ? logoMatch[1] : 'https://via.placeholder.com/200x280/333333/666666?text=EPIC+MOVIE';
...
function displayMovies(channels) {
    ...
    channels.forEach(channel => {
        ...
        movieElement.innerHTML = `
            <img src="${channel.logo}" alt="${channel.name}" class="movie-logo" 
                 onerror="this.src='https://via.placeholder.com/200x280/333333/666666?text=EPIC+MOVIE'">
            <div class="movie-name">${channel.name}</div>
        `;
